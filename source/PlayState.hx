...
class PlayState extends MusicBeatState
{
	...
	public static var songScore2:Int = 0;
	public static var songHits2:Int = 0;
	public static var songMisses2:Int = 0;
	...
	// FIRE!!!!!!!!!!!!!!!!!
	public static var threedlist:Array<String> = ['joeseph', 'joespin', 'joeseph-pissy', 'joeseph-w'];
	public static var floatY:Array<String> = ['joeseph', 'joespin', 'joeseph-pissy', 'joeseph-w'];
	public static var floatX:Array<String> = [''];
	public static var threedcharacter:Bool = false;
	public static var threedplayer:Bool = false;

	var noMovementOnNoteHit:Array<String> = ['joeseph', 'joespin'];
	...
	override public function create()
	{
		...
		if(threedlist.contains(dad.curCharacter.toLowerCase()))
		{
			threedcharacter = true;
		}

		if(threedlist.contains(boyfriend.curCharacter.toLowerCase()))
		{
			threedplayer = true;
		}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (gottaHitNote){ // for the 3d note thing
		...
		switch(event.event) {
			case 'Change Character':
				...
				switch(event.value1.toLowerCase()) {
					case 'gf' | 'GF' | 'girlfriend' | '1':
						charType = 2;
					case 'dad' | 'Dad' | 'opponent' | '0':
						charType = 1;
					default:
	...
	override public function update(elapsed:Float)
	{
		...
		if(floatX.contains(dad.curCharacter.toLowerCase()))
		{
			dad.x += (Math.cos(elapsedtime) * 0.6);
		}
		if(floatX.contains(boyfriend.curCharacter.toLowerCase()))
		{
			boyfriend.x += (Math.cos(elapsedtime) * 0.6);
		}

		if(floatY.contains(dad.curCharacter.toLowerCase()))
		{
			dad.y += (Math.sin(elapsedtime) * 0.6);
		}
		if(floatY.contains(boyfriend.curCharacter.toLowerCase()))
		{
...
for (key => value in luaShaders)
{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(char != null)
			{
				...
				if (!noMovementOnNoteHit.contains(dad.curCharacter.toLowerCase())) {
					var camPower = 60;
					moveCameraSection();
					if (!SONG.notes[curSection].mustHitSection) {
			...
			if(!note.noAnimation) {
				...
				if(note.gfNote)
				{
					if(gf != null)
					{
						...
						if (!noMovementOnNoteHit.contains(gf.curCharacter.toLowerCase())) {
							var camPower = 60;
							moveCameraSection();
							if (SONG.notes[curSection].mustHitSection) {
								switch (note.noteData) {
				...
				else
				{
					...
					if (!noMovementOnNoteHit.contains(boyfriend.curCharacter.toLowerCase())) {
						var camPower = 60;
						moveCameraSection();
						if (SONG.notes[curSection].mustHitSection) {
							switch (note.noteData) {
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		songScore2 = songScore;
		songMisses2 = songMisses;
		songHits2 = songHits;
